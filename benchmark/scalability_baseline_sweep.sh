...
run_one_config() {
    ...
    for RETRY in $(seq 1 "$RETRIES"); do
        ...
        local FAB_CMD
        FAB_CMD="${LOCAL_ORCH:+LOCAL_ORCH=$LOCAL_ORCH }ROUTING_MODE=baseline NODES=$NODES_VAL RATE=$RATE_VAL DURATION=$DURATION WARMUP=$WARMUP IN_MEMORY_STORE=1 fab cloudlab --manifest=$MANIFEST --username=$USERNAME --latency=$LATENCY --primary-bw=$PRIMARY_BW --worker-bw=$WORKER_BW --log-dir=$RUN_DIR"
