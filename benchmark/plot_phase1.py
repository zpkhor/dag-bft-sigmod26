...
def plot_phase1(results_dir: Path, output_path: Optional[Path], smooth: bool) -> None:
    groups = discover_runs(results_dir)
    if not groups:
        raise ValueError(
            f"No rate_*_bw_*mbit_routing_*_run_* subdirectories found in {results_dir}"
        )
    ...
    for col_idx, (rate, bw_mbit, routing_mode) in enumerate(columns):
        runs = groups[(rate, bw_mbit, routing_mode)]
        metrics_lines = []
        ...
        col_title = f"rate={rate}  bw={bw_mbit}mbit  routing={routing_mode}"
    ...
    if output_path is None:
        ...
        output_path = benchmark_dir / f"{results_dir.name}-phase1.png"
...
def main() -> int:
    ...
    try:
        plot_phase1(results_dir, output_path, not args.no_smooth)
