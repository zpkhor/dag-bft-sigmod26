...
def plot_phase3_cloudlab(results_dir: Path, output_path: Optional[Path], smooth: bool) -> None:
    groups = discover_runs(results_dir)
    assert groups, (
        f"No *_run_* subdirectories found in {results_dir}"
    )
    ...
    if output_path is None:
        ...
        output_path = benchmark_dir / f"{results_dir.name}-phase3-cloudlab.png"
...
def main() -> int:
    ...
    plot_phase3_cloudlab(results_dir, output_path, not args.no_smooth)
    return 0
