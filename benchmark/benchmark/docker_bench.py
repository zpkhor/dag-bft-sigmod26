...
def egress_bw_capacity_tps(bw_mbit, tx_size, num_peers):
    ...
    return int(bw_bytes / (tx_size + 44) / num_peers * 0.8)
...
class DockerBench:
    ...
    # Match CloudLab TCP buffer tuning (cloudlab_bench.py:123-136).
    # The host's net.core.rmem_max/wmem_max (often ~208KB) caps the TCP window,
    # limiting per-connection throughput on high-RTT netem links.
    TCP_SYSCTLS = (
        "    sysctls:\n"
    ...
    def __init__(
        ...
        self.numa_node_cpus = numa_node_cpus

        if numa_node_cpus > 0 and cpus_per_validator > 0 and num_numa_nodes > 0 and num_executors > 0:
            per_numa = numa_node_cpus // cpus_per_validator
            client_numa = 1 + (nodes + per_numa - 1) // per_numa
            exec_numa_nodes = num_numa_nodes - (client_numa + 1)
            max_exec_containers = exec_numa_nodes * numa_node_cpus // self.EXEC_CPUS_PER
            max_num_executors = max_exec_containers // nodes
            assert num_executors <= max_num_executors, (
                f"num_executors={num_executors} exceeds capacity {max_num_executors} "
                f"({exec_numa_nodes} executor NUMA nodes × {numa_node_cpus // self.EXEC_CPUS_PER} executors/node "
                f"// {nodes} validators)"
            )
    ...
    def _container_ip(self, i):
        return f"172.22.0.{10 + i}"

    @staticmethod
    def _docker_down(compose_file='docker-compose.yml'):
        try:
            subprocess.run(
                [
                    "docker",
    ...
    def _generate_compose(self, nodes, commands_per_validator, wait_ports_per_validator, client_command, client_ip, container_ips, client_wait_ports, primary_ports_str, logs_host_path, compose_file, executor_commands=None, executor_ips=None):
        ...
        for i in range(nodes):
            ...
            if self.cpus_per_validator > 0:
                if self.numa_node_cpus > 0:
                    per_numa = self.numa_node_cpus // self.cpus_per_validator
                    # Skip NUMA node 0: system interrupts, Docker daemon, and
                    # bridge softirqs run there and preempt validator threads.
                    numa_node = 1 + i // per_numa
                    offset = (i % per_numa) * self.cpus_per_validator
                    start = numa_node * self.numa_node_cpus + offset
                else:
        ...
        if executor_commands and executor_ips:
            ...
            for i in range(nodes):
                for e in range(self.num_executors):
                    ...
                    e_cpuset = ""
                    if self.cpus_per_validator > 0:
                        # Non-NUMA: validators [0, nodes*slot), client [nodes*slot, nodes*slot+2*nodes), executors after client
                        # NUMA: validators on nodes 1.., client on next NUMA node, executors on NUMA node after client
                        exec_cpus_per = self.EXEC_CPUS_PER
                        if self.numa_node_cpus > 0:
                            per_numa = self.numa_node_cpus // self.cpus_per_validator
                            client_numa = 1 + (nodes + per_numa - 1) // per_numa
                            exec_numa_start = (client_numa + 1) * self.numa_node_cpus
                            e_start = exec_numa_start + exec_idx * exec_cpus_per
                        else:
                            client_cpus = 2 * nodes
                            e_start = nodes * self.cpus_per_validator + client_cpus + exec_idx * exec_cpus_per
    ...
    environment:
      ...
      - TC_EXECUTOR_BW=10gbit"""
        ...
        if self.cpus_per_validator > 0:
            if self.numa_node_cpus > 0:
                per_numa = self.numa_node_cpus // self.cpus_per_validator
                # Place client on the NUMA node after all validators (skip node 0)
                client_numa = 1 + (nodes + per_numa - 1) // per_numa
                c_start = client_numa * self.numa_node_cpus
                client_cpus = self.numa_node_cpus  # give client a full NUMA node
            else:
                c_start = nodes * self.cpus_per_validator
                client_cpus = 2 * nodes
            client_cpuset = f'\n    cpuset: "{c_start}-{c_start + client_cpus - 1}"'
...
{self.TCP_SYSCTLS}
    ...
    volumes:
      - {os.path.abspath('benchmark_client')}:/app/benchmark_client:ro
      - {logs_host_path}:/logs:rw
    environment:
...
"""
    ...
    def run(self, debug=False, log_dir=None):
        ...
        # Match CloudLab TCP buffer tuning (cloudlab_bench.py:123-136).
        # net.core.rmem_max/wmem_max are not network-namespaced, so they must
        # be set on the host. The host default (~208KB) caps TCP windows well
        # below what the tc bandwidth shaping allows on high-RTT netem links.

        # To revert:
        # sudo sysctl -w net.core.rmem_max=212992 net.core.wmem_max=212992 net.core.netdev_max_backlog=1000
        # subprocess.run(
        #     'sudo sysctl -w '
        ...
        if log_dir:
            ...
            logs_host_path = os.path.abspath(log_dir)
        else:
        ...
        compose_file = os.path.join(log_dir, 'docker-compose.yml') if log_dir else 'docker-compose.yml'
        ...
        try:
            ...
            for i in range(nodes):
                for e in range(self.num_executors):
                    executor_ips.append(f"172.22.0.{10 + nodes + 1 + i * self.num_executors + e}")
            client_ip = f"172.22.0.{10 + nodes + 1 + nodes * self.num_executors}"
            ...
            db_base = log_dir if log_dir else '.'
            for i in range(nodes):
                subprocess.run(f"mkdir -p {os.path.join(db_base, f'.db-{i}')}", shell=True)
                for w in range(self.workers):
                    subprocess.run(f"mkdir -p {os.path.join(db_base, f'.db-{i}-{w}')}", shell=True)
            ...
            if self.num_executors > 0:
                ...
                committee.set_client_reply_addresses({i: f'{client_ip}:{exec_reply_port}' for i in range(nodes)})
            ...
            self._generate_compose(good_nodes, commands_per_validator, wait_ports_per_validator, client_command, client_ip, container_ips, client_wait_ports, primary_ports_str, logs_host_path, compose_file, executor_commands=executor_commands, executor_ips=executor_ips)
            ...
            self._docker_down(compose_file)
            sleep(1)
