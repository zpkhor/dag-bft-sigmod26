...
class CloudLabInstanceManager:
    ...
    @classmethod
    def make(cls, manifest_file=None, username=None, ban_file='cloudlab_ban.txt',
             local_orch=False):
        from benchmark.manifest import Manifest
        if local_orch:
            manifest = Manifest.load_from_etc_hosts()
        else:
            assert manifest_file is not None, 'manifest_file required when local_orch=False'
            assert username is not None, 'username required when local_orch=False'
