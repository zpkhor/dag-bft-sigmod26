#!/usr/bin/env bash
# Motivation figure sweep: validator and executor layer imbalance scenarios.
# Covers both layers and their interaction. Replaces fig_1.1_motivation_validator_run.sh.
# Supports both docker and cloudlab via MODE env var.
# Fixed: BASELINE=1 NUM_EXECUTORS=3 NO_SEND_PAYMENT=1 IN_MEMORY_STORE=1
...
RESULTS_DIR="$SCRIPT_DIR/results/motivation"
if [[ "$MODE" == "docker" ]]; then
...
CONFIGS=(
    ...
    "n4_balanced|600,600,600,600|1,1,1,1|1,1,1|17000,50000,75000,95000,110000,117500,125000,135000,137000"

    # 2. 90% load on first validator (saturates ~70k)
    "n4_v_rate_imb90|600,600,600,600|27,1,1,1|1,1,1|20000,35000,40000,45000,50000,62000,70000"

    # 3. 60% load on first validator (saturates ~80k)
    # "n4_v_rate_imb60|600,600,600,600|4.5,1,1,1|1,1,1|30000,50000,68000,80000,90000,100000"

    # 4. 90% load on first executor (saturates ~65k, TBD)
    "n4_e_rate_imb90|600,600,600,600|1,1,1,1|18,1,1|20000,29000,40000,45000,50000,55000,65000,73000,77000,82000"

    # 5. 60% load on first executor (saturates ~79k)
    # "n4_e_rate_imb60|600,600,600,600|1,1,1,1|3,1,1|30000,48000,62000,73000,82000,88000,94000,100000"

    # 6. 90% load on first validator + 90% on first executor (saturates ~60k, TBD)
    "n4_ve_rate_imb90|600,600,600,600|27,1,1,1|18,1,1|20000,33000,45000,55000,62000,68000"

    # 7. 60% load on first validator + 60% on first executor (saturates ~70k, TBD)
    # "n4_ve_rate_imb60|600,600,600,600|4.5,1,1,1|3,1,1|25000,42000,60000,63000,66000,69000,72000,82000,86000"

    # 8. f=1 low-bw validator (200mbit)
    "n4_bw_f|600,600,600,200|1,1,1,1|1,1,1|20000,33000,45000,54000,60000,64000,68000,72000"
...
for CONFIG in "${CONFIGS[@]}"; do
    ...
    for RATE in "${RATES[@]}"; do
        for RETRY in $(seq 1 "$RETRIES"); do
            ...
            if [[ "$MODE" == "docker" ]]; then
                FAB_CMD="NODES=4 WORKER_BANDWIDTHS_MBPS=$BANDWIDTHS_MBPS RATE_WEIGHTS=$RATE_WEIGHTS E_SKEW_WEIGHTS=$E_SKEW_WEIGHTS BASELINE=1 NEW_SCHEDULER=0 NO_SEND_PAYMENT=1 NUM_EXECUTORS=3 RATE=$RATE DURATION=$DURATION WARMUP=$WARMUP IN_MEMORY_STORE=1 fab docker --cpus-per-validator=$CPUS_PER_VALIDATOR --latency=$LATENCY --primary-bw=$PRIMARY_BW --log-dir=$RUN_DIR"
            elif [[ "$MODE" == "cloudlab" ]]; then
...
echo ""
echo "==========================================="
echo "Motivation sweep complete. Results in $RESULTS_DIR"
