...
def docker(ctx, debug=False, worker_bw='600mbit', latency='100ms', jitter='0ms',
    ...
    num_numa_nodes = int(os.environ.get('NUM_NUMA_NODES', 8))
    bench_params = {
        'faults': 0,
        'nodes': nodes,
        'workers': 1,
...
def cloudlab(ctx, debug=False,
             manifest='manifest.xml', username='anon', latency='100ms', worker_bw='600mbit',
...
@task
def cloudlab_install(ctx, manifest='manifest.xml', username='anon'):
...
@task
def cloudlab_check(ctx, manifest='manifest.xml', username='anon', timeout=10):
...
@task
def cloudlab_kill(ctx, manifest='manifest.xml', username='anon'):
...
@task
def cloudlab_get_ssh(ctx, manifest='manifest.xml', username='anon'):
...
@task
def cloudlab_nettest(ctx, manifest='manifest.xml', username='anon'):
...
@task
def cloudlab_pingtest(ctx, manifest='manifest.xml', username='anon'):
