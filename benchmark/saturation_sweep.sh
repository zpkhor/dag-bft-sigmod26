#!/usr/bin/env bash
# Saturation sweep: BASELINE=1 always. Sweep rates for balanced/imbalanced configs
# to find throughput under each scenario.
...
for CONFIG in "${CONFIGS[@]}"; do
    ...
    for RATE in "${RATES[@]}"; do
        for RETRY in $(seq 1 "$RETRIES"); do
            ...
            if [[ "$MODE" == "docker" ]]; then
                FAB_CMD="NODES=$NODES_VAL WORKER_BANDWIDTHS_MBPS=$BANDWIDTHS_MBPS RATE_WEIGHTS=$RATE_WEIGHTS BASELINE=1 RATE=$RATE DURATION=$DURATION WARMUP=$WARMUP fab docker --cpus-per-validator=$CPUS_PER_VALIDATOR --latency=$LATENCY --primary-bw=$PRIMARY_BW --log-dir=$RUN_DIR"
            elif [[ "$MODE" == "cloudlab" ]]; then
                FAB_CMD="${LOCAL_ORCH:+LOCAL_ORCH=$LOCAL_ORCH }NODES=$NODES_VAL WORKER_BANDWIDTHS_MBPS=$BANDWIDTHS_MBPS RATE_WEIGHTS=$RATE_WEIGHTS BASELINE=1 RATE=$RATE DURATION=$DURATION WARMUP=$WARMUP fab cloudlab --manifest=$MANIFEST --latency=$LATENCY --primary-bw=$PRIMARY_BW --log-dir=$RUN_DIR"
            else
...
echo ""
echo "==========================================="
echo "Saturation sweep complete. Results in $RESULTS_DIR"
