...
pub struct Committee {
    ...
    /// Client reply addresses keyed by region_id (u8).
    #[serde(default)]
...
impl Committee {
    ...
    /// Returns the reply address for a specific region_id.
    pub fn client_reply_address(&self, region_id: u8) -> Option<&SocketAddr> {
        self.client_reply_addresses.get(&region_id)
    }
...
/// Creates an initial Partition using per-validator range sharding.
///
/// Each validator's account range is divided into `num_executors` equal sub-shards,
/// assigned to executors in order: sub-shard 0 → executor 0, sub-shard 1 → executor 1, etc.
/// This means each executor owns non-contiguous slices across validators, but the sub-shard
/// boundaries within each validator align with the client's E_SKEW_WEIGHTS sub-ranges,
/// allowing independent control of validator-level skew (RATE_WEIGHTS) and executor-level
/// skew (E_SKEW_WEIGHTS).
