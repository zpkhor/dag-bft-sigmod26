...
impl DistributedTxExecutor {
    ...
    async fn execute_single_local_transaction( 
        ...
        let client_addr = match self.committee.client_reply_address(tx_id.client_id) {
            Some(addr) => *addr,
            None => {
                warn!("Client ID {} not found in committee, skipping reply", tx_id.client_id);
                return false;
            }
        };
    ...
    async fn execute_batch(&mut self, digest: &Digest, transactions: &[Vec<u8>]) {
        ...
        // Transaction format: [tx_type:1][client_id:1][tx_counter:8][workload_payload:variable][padding:size in client]
        //
