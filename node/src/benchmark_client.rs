...
async fn main() -> Result<()> {
    let matches = App::new(crate_name!())
        ...
        .args_from_usage("--balanced-rate=[INT] 'Balanced rate per region for warmup phase (default 0)'")
        ...
        .args_from_usage("--start-at=[FLOAT] 'Unix timestamp (seconds) to begin sending'")
    ...
    let balanced_rate: u64 = matches
        .value_of("balanced-rate")
        .unwrap_or("0")
        .parse::<u64>()
        .context("--balanced-rate must be a non-negative integer")?;
    ...
    let start_at: Option<f64> = matches
        .value_of("start-at")
        .map(|x| x.parse::<f64>())
        .transpose()
        .context("--start-at must be a float (Unix timestamp)")?;
    ...
    let own_region_id = {
        let mut entries: Vec<(u64, &PublicKey)> = account_ranges
            .iter()
            .map(|(pk, &(start, _))| (start, pk))
            .collect();
        entries.sort_by_key(|&(start, _)| start);
        entries.iter()
            .position(|(_, pk)| *pk == &own_validator)
            .expect("own_validator must be in account_ranges")
    };
    let (own_start, own_count) = account_ranges[&own_validator];
    info!(
        "Region {}: accounts {}..{} ({} accounts), rate_weight={}",
        own_region_id, own_start, own_start + own_count - 1, own_count, rate
    );
    ...
    info!(
        "no_send_payment={}, zipf_exponent={}, num_tx_types={}, executor_skew_weights={:?}",
        no_send_payment, zipf_exponent, num_tx_types, executor_skew_weights
    );
...
impl Client {
    async fn connect_all_validators(&self) -> Result<HashMap<PublicKey, Vec<mpsc::UnboundedSender<Bytes>>>> {
        ...
        for pk in self.sorted_validators.iter() {
    ...
    pub async fn send(&self) -> Result<()> {
        ...
        // One routing table per region (validator account range).
        let routing_tables: HashMap<PublicKey, Arc<RwLock<HashMap<u64, PublicKey>>>> =
            self.sorted_validators
                .iter()
                .copied()
                .map(|pk| (pk, Arc::new(RwLock::new(HashMap::new()))))
                .collect();

        let account_ranges_by_region: Vec<(u64, u64, PublicKey)> = self
            .sorted_validators
            .iter()
            .map(|pk| {
                let (start, count) = self.account_ranges[pk];
                (start, count, *pk)
            })
            .collect();
        ...
        if let Some(reply_addr) = self.reply_addr {
            let rt = routing_tables.clone();
            let ranges = account_ranges_by_region.clone();
            let f_plus_one = self.f_plus_one;
            tokio::spawn(async move {
                if let Err(e) = migration_listener(reply_addr, rt, ranges, f_plus_one).await {
        ...
        if let Some(exec_addr) = self.execution_reply_addr {
            let f_plus_one = self.f_plus_one;
            tokio::spawn(async move {
                if let Err(e) = execution_reply_listener(exec_addr, f_plus_one).await {
        ...
        let region_id = {
            let mut entries: Vec<(u64, PublicKey)> = self.account_ranges
                .iter()
                .map(|(pk, &(start, _))| (start, *pk))
                .collect();
            entries.sort_by_key(|&(start, _)| start);
            entries.iter()
                .position(|(_, pk)| pk == &self.own_validator)
                .expect("own_validator must be in account_ranges") as u8
        };
        let (acct_start, acct_count) = self.account_ranges[&self.own_validator];
        assert!(acct_count > 0, "account count for region {} must be > 0", region_id);

        // Map each validator PK to its region id (sorted by account range start, same as region_id above).
        let pk_to_region: HashMap<PublicKey, u8> = {
            let mut entries: Vec<(u64, PublicKey)> = self.account_ranges
                .iter()
                .map(|(pk, &(start, _))| (start, *pk))
                .collect();
            entries.sort_by_key(|&(start, _)| start);
            entries.iter().enumerate().map(|(i, (_, pk))| (*pk, i as u8)).collect()
        };

        let validator_senders = self.connect_all_validators().await?;
        let rt = routing_tables
            .get(&self.own_validator)
            .expect("missing routing table for own_validator")
            .clone();
        ...
        send_shard(
            ...
            pk_to_region,
            self.num_tx_types,
            self.executor_skew_weights.clone(),
            total_num_accounts,
            self.rampup_secs,
            self.start_at,
        ).await
...
async fn send_shard(
    ...
    size: usize,
    region_id: u8,
    pk_to_region: HashMap<PublicKey, u8>,
    num_tx_types: u8,
    executor_skew_weights: Vec<f64>,
    ...
    let ramp1_ticks = rampup_ticks / 10;
    ...
    // Sub-ranges divide [account_start, account_start + num_accounts) into equal slices that
    // align with executor partition boundaries from create_initial_partition (which shards each
    // validator's range into num_executors equal sub-shards). This alignment means E_SKEW_WEIGHTS
    // controls per-executor transaction skew independently of RATE_WEIGHTS (validator-level skew).
    ...
    // Wait for synchronized wall-clock start time if specified.
    if let Some(start_ts) = start_at {
        use std::time::{SystemTime, UNIX_EPOCH};
        let now_epoch = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs_f64();
        assert!(start_ts > now_epoch, "start-at {} is in the past (now={})", start_ts, now_epoch);
        let wait = Duration::from_secs_f64(start_ts - now_epoch);
        info!("Waiting {:.2}s until start-at {:.3}", wait.as_secs_f64(), start_ts);
        sleep(wait).await;
    }
    ...
    'main: loop {
        ...
        // Warmup phase 1 (first 1/10): ramp balanced_rate from 0 using quadratic.
        // Warmup phase 2 (last 2/3): hold steady at balanced_rate.
        // After warmup: switch to configured rate (rate_weights applied).
        // Offsets ensure total_sent is continuous at both transitions.
        let next_total = if rampup_ticks > 0 && counter < ramp1_ticks {
            balanced_rate * (counter + 1) * (counter + 1) / (2 * ramp1_ticks * PRECISION)
        } else if rampup_ticks > 0 && counter < rampup_ticks {
            let offset1 = balanced_rate * ramp1_ticks / (2 * PRECISION);
            offset1 + balanced_rate * (counter + 1 - ramp1_ticks) / PRECISION
        } else {
            let offset2 = balanced_rate * (rampup_ticks - ramp1_ticks / 2) / PRECISION;
            offset2 + rate * (counter + 1 - rampup_ticks) / PRECISION
        };
        if rampup_ticks > 0 && counter == rampup_ticks {
            // NOTE: This log entry is used to compute performance.
            info!("Balanced phase ended region {}", region_id);
        }
        ...
        for x in 0..burst {
            ...
            let (senders, target_region_id): (&Vec<mpsc::UnboundedSender<Bytes>>, u8) = if round_robin {
                let v_idx = validator_rr.entry(src_account_id)
                    .or_insert((src_account_id as usize) % num_validators);
                let idx = *v_idx;
                let target_pk = sorted_validators[idx];
                *v_idx = (idx + 1) % num_validators;
                let r = pk_to_region.get(&target_pk).copied().unwrap_or(region_id);
                (&validator_senders[&target_pk], r)
            } else {
                let target_pk = routing_table.read().unwrap().get(&src_account_id).copied();
                match target_pk {
                    Some(pk) if pk != own_validator => {
                        let r = pk_to_region.get(&pk).copied().unwrap_or(region_id);
                        (validator_senders.get(&pk).unwrap_or(own_senders), r)
                    }
                    _ => (own_senders, region_id),
                }
            };
            ...
            // SmallBank format: [tx_type:1][region_id:1][tx_counter:8][src:8][dest:8][sb_type:1][amount:8] = 35 bytes
            ...
            if is_sample {
                // NOTE: This log entry is used to compute performance.
                info!("Sending sample transaction {} account {} from region {} to region {}", counter, src_account_id, region_id, target_region_id);
            }
...
async fn migration_listener(
    addr: SocketAddr,
    routing_tables: HashMap<PublicKey, Arc<RwLock<HashMap<u64, PublicKey>>>>,
    account_ranges_by_region: Vec<(u64, u64, PublicKey)>,
    f_plus_one: usize,
) -> Result<()> {
    ...
    let routing_tables = Arc::new(routing_tables);
    let account_ranges_by_region = Arc::new(account_ranges_by_region);
    ...
    loop {
        ...
        let account_ranges_by_region = account_ranges_by_region.clone();
                        ...
                        match bincode::deserialize::<MigrationMessage>(&data) {
                            Ok(msg) => {
                                ...
                                if !confirmed.is_empty() {
                                    let mut updates_by_region: HashMap<PublicKey, Vec<(u64, PublicKey)>> =
                                        HashMap::new();

                                    for (account_id, new_target) in &confirmed {
                                        if let Some(region_pk) =
                                            find_region_for_account(*account_id, &account_ranges_by_region)
                                        {
                                            updates_by_region
                                                .entry(region_pk)
                                                .or_default()
                                                .push((*account_id, *new_target));
                                        } else {
                                            warn!(
                                                "No region range contains migrated account {}",
                                                account_id
                                            );
                                        }
                                    }
                                    ...
                                    for (region_pk, updates) in updates_by_region {
                                        if let Some(rt) = routing_tables.get(&region_pk) {
                                            let mut table = rt.write().unwrap();
                                            for (account_id, new_target) in updates {
                                                table.insert(account_id, new_target);
                                                rerouted += 1;
                                            }
                                        } else {
                                            warn!(
                                                "Missing routing table for region validator {}",
                                                region_pk
                                            );
                                        }
                                    }

                                    let total_routed: usize = routing_tables
                                        .values()
                                        .map(|rt| rt.read().unwrap().len())
                                        .sum();
...
fn find_region_for_account(
    account_id: u64,
    account_ranges_by_region: &[(u64, u64, PublicKey)],
) -> Option<PublicKey> {
    account_ranges_by_region.iter().find_map(|(start, count, region_pk)| {
...
async fn execution_reply_listener(
    ...
    // Track replies per (region_id, tx_counter): set of responding validator public keys
    let reply_counts: Arc<Mutex<HashMap<(u8, u64), HashSet<PublicKey>>>> =
                ...
                match result {
                    Ok(data) => {
                        match bincode::deserialize::<ClientReply>(&data) {
                            Ok(reply) => {
                                if reply.tx_type != SAMPLE_TX_TYPE {
                                    continue;
                                }

                                let mut counts = reply_counts.lock().unwrap();
                                let entry = counts
                                    .entry((reply.region_id, reply.tx_counter))
                                    .or_default();
                                ...
                                if count == f_plus_one {
                                    // NOTE: This log entry is used to compute performance.
                                    info!(
                                        "Sample transaction {} account {} completed with {} confirmations",
                                        reply.tx_counter, reply.account_id, count
                                    );
                                }
                            }
                            Err(e) => {
