...
pub struct Proposer {
    ...
    rx_workers: Receiver<(Digest, WorkerId, HashMap<u32, u16>, u64)>,
    ...
    /// Accumulated queue delay sum and count for batches pending in the next header.
    qd_sum: u64,
    qd_count: u64,
}
...
impl Proposer {
    ...
    async fn make_header(&mut self) {
        ...
        let avg_queue_delay_ms = if self.qd_count > 0 { self.qd_sum / self.qd_count } else { 0 };
        self.qd_sum = 0;
        self.qd_count = 0;
    ...
    pub async fn run(&mut self) {
        ...
        loop {
            ...
            tokio::select! {
                ...
                Some((digest, worker_id, counts, queue_delay_ms)) = self.rx_workers.recv() => {
                    self.payload_size += digest.size();
                    self.qd_sum += queue_delay_ms;
                    self.qd_count += 1;
