...
pub struct Header {
    ...
    /// Per-account tx counts aggregated across all batches in this header's payload.
    /// Committed in the header digest (sorted by key) so a Byzantine proposer cannot send
    /// different account_counts for the same header.id to different peers — equivocating values
    /// would produce inconsistent migration decisions in consensus. Correctness (that the value
    /// matches the actual batch contents) is separately enforced by core.rs before voting.
    pub account_counts: HashMap<u32, u16>,
...
impl Header {
    pub async fn new(
        ...
        account_counts: HashMap<u32, u16>,
        avg_queue_delay_ms: u64,
...
impl Hash for Header {
    fn digest(&self) -> Digest {
        ...
        let mut sorted_counts: Vec<(u32, u16)> = self.account_counts.iter().map(|(&k, &v)| (k, v)).collect();
        sorted_counts.sort_unstable_by_key(|&(k, _)| k); // keys are unique (HashMap), so unstable sort is deterministic
        for (account_id, count) in sorted_counts {
            hasher.update(account_id.to_le_bytes());
            hasher.update(count.to_le_bytes());
        }
        hasher.update(self.avg_queue_delay_ms.to_le_bytes());
